  ...
  useEffect(() => {
    if (previousOrders.length > 0 && !isLoading) {
      ...
      setFormData((prevData) => ({
        ...prevData,
        customerInfo: {
          name: latestOrder.customerId?.name || prevData.customerInfo.name,
          email: latestOrder.customerId?.email || prevData.customerInfo.email,
          phone: latestOrder.customerId?.phone || prevData.customerInfo.phone,
          address: {
            street:
              latestOrder.customerId?.address?.street ||
              prevData.customerInfo.address.street,
            city:
              latestOrder.customerId?.address?.city ||
              prevData.customerInfo.address.city,
            state:
              latestOrder.customerId?.address?.state ||
              prevData.customerInfo.address.state,
            postalCode:
              latestOrder.customerId?.address?.postalCode ||
              prevData.customerInfo.address.postalCode,
            country:
              latestOrder.customerId?.address?.country ||
